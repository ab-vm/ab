...
template <typename Type> class ArrayCell : public Cell {
public:
	/// Array indexing.
...
template <typename Type> inline Type& ArrayCell<Type>::operator[](std::size_t n) {
	return value_[n];
}
...
template <typename Type> inline ArrayCell<Type>::operator Type*() {
	return value_;
}

template <typename Type> constexpr inline ArrayCell<Type>::operator const Type*() const {
	return value_;
}

template <typename Type> template <typename Visitor> void ArrayCell<Type>::visit(Visitor& visitor) {
	for (auto&& x : *this) {
		visitor.visit();
	}
}
