...
private:
	/**
	 * OMR requires that the language reserve the least significant byte in the first
	 * fomrobject_t slot of an object to record object flag bits used in generational and
...
private:
	 ...
	 */
	uintptr_t extractSizeFromObjectHeaderSlot(fomrobject_t headerSlot) {
...
public:
	...
	/**
	 * Get the exact size of the object header, in bytes. This includes the size of the metadata
	 * slot.
	 */
	...
	/**
	 * Get the exact size of the object data, in bytes. This excludes the size of the object
	 * header and any bytes added for object alignment. If the object has a discontiguous
	 * representation, this method should return the size of the root object that the
	...
	/**
	 * Get the exact size of the object, in bytes, including the object header and data. This
	 * should not include any padding bytes added for alignment. If the object has a
	 * discontiguous representation, this method should return the size of the root object that
	 * the discontiguous parts depend from.
	 *
	 * @param[in] objectPtr points to the object to determine size for
	...
	MMINLINE uintptr_t getObjectSizeInBytesWithHeader(omrobjectptr_t objectPtr) {
		fomrobject_t* headerSlotAddress =
			(fomrobject_t*)objectPtr + _objectHeaderSlotOffset;
	...
	/**
	 * If object initialization fails for any reason, this method must return NULL. In that
	 * case, the heap memory allocated for the object will become floating garbage in the heap
	 * and will be recovered in the next GC cycle.
	 *
	 ...
	 * @param[in] allocateInitialization points to the MM_AllocateInitialization instance used
	 * to allocate the heap memory
	...
	/**
	 * Returns TRUE if an object is indexable, FALSE otherwise. Languages that support indexable
	 * objects (e.g. arrays) must provide an implementation that distinguishes indexable from
	 * scalar objects.
	 *
 ...
 */
	 ...
	 *
	 * @param forwardedHeader pointer to the MM_ForwardedHeader instance encapsulating the
	 * object
	 * @return TRUE if object is indexable, FALSE otherwise
	 */
	...
	/**
	 * Get the instance size (total) of a forwarded object from the forwarding pointer. The
	 * size must include the header and any expansion bytes to be allocated if the object will
	 * grow when moved.
	 *
	 * @param[in] forwardedHeader pointer to the MM_ForwardedHeader instance encapsulating the
	 * object
	...
	/**
	 * Return true if the object holds references to heap objects not reachable from reference
	 * graph. For example, an object may be associated with a class and the class may have
	 * associated meta-objects that are in the heap but not directly reachable from the root
	 * set. This method is called to determine whether or not any such objects exist.
	 *
	 ...
	 */
	MMINLINE bool hasIndirectObjectReferents(
		CLI_THREAD_TYPE* thread, omrobjectptr_t objectPtr) {
		return false;
	}

	/**
	 * Calculate the actual object size and the size adjusted to object alignment. The
	 * calculated object size includes any expansion bytes allocated if the object will grow
	 * when moved.
	 *
	 * @param[in] env points to the environment for the calling thread
	 * @param[in] forwardedHeader pointer to the MM_ForwardedHeader instance encapsulating the
	 ...
	 * @param[out] hotFieldAlignmentDescriptor pointer to hot field alignment descriptor for
	 * class (or NULL)
	 */
