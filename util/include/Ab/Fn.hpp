...
class FnRef<R(Args...)> {
public:
    FnRef(void* data, InvokeFn<R, Args...> invoke)
        : data_(data), invoke_(invoke) {}
...
class FnInterface {
public:
    ~FnInterface() noexcept {
        table_->destruct(this);
    }

    R invoke(Args... args) const {
        return table_->invoke(this, std::forward<Args>(args)...);
    }

protected:
    constexpr FnInterface(const FnTable<R, Args...>* table)
        : table_(table) {}
...
private:
    static R do_invoke(const void* interface, Args... args) {
        return static_cast<const FnStorage*>(interface)->value_(std::forward<Args>(args)...);
    }
...
public:
    ...
    R invoke(Args... args) const {
        return interface_->invoke(std::forward<Args>(args)...);
    }
...
private:
    template <typename T>
    T* cast_storage() noexcept { return reinterpret_cast<T*>(storage_); }

    template <typename T>
    const T* cast_storage() const noexcept { return reinterpret_cast<const T*>(storage_); }

    const FnInterface<R, Args...>* get_interface() const noexcept { return cast_storage<FnInterface<R, Args...>>(); }
